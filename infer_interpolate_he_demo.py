...
parser.add_argument('--infer_name', required=True)
parser.add_argument('--gen_no', type=int, required=True)
parser.add_argument('--input_char', type=str, required=True)
...
def main():
    ...
    if args.from_txt:
        ...
        # # For AI4Future Lab Demo
        img_list = [transforms.Normalize(0.5, 0.5)(
            transforms.ToTensor()(
                # yf_input_original_char('./useful_functions/data/和-宋徽宗.jpg')
                # yf_input_original_char('./useful_functions/data/于右任-標準草書千字文-和-行書五言聯1.png')
                # yf_input_original_char('./useful_functions/data/于右任-標準草書千字文-和-vector1.png')
                # yf_input_original_char('./useful_functions/data/蘇東坡-和.jpeg')
                yf_input_original_char(input_char)
        )
...
def infer_interpolate_yf(result_folder, infer_name, label):
    ...
    resume = 100000
    from_txt = 1
    src_font = "./fonts/source/仓耳今楷03-W03.ttf"
    src_txt = "和"
   ...
   # python infer_interpolate_yf.py -- -- --from_txt --src_font ./fonts/source/仓耳今楷03-W03.tt--src_txt 和 --label
    ...
    global_steps = 0
    with open(type_file, 'r', encoding='utf-8') as fp:
        fonts = [s.strip() for s in fp.readlines()]
    writer_dict = {v: k for k, v in enumerate(fonts)}
    ...
    for batch in dataloader:
        ...
        style = model.sample_demo_yf(batch, infer_dir, result_folder, infer_name)
        global_steps += 1

        print(batch[0])  # yf: the label
